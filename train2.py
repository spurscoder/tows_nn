...
def para():
    ...
    global id2label
    a = set()
    with open("/home/user2/webservicefile/ws2.jl.txt", "r") as f:
...
def load_file(filename):
    ...
    with open(filename, "r") as f:
        for item in json_lines.reader(f):
            for tag in item['tag']:
                apiname, apiintro, label = item["name"], item["intro"], tag
            #                 d=[]
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_acc > self.best_val_acc:
            ...
            model.save_weights("tagdetect_best_model.weights")
...
def main():
    para()
    wsdata = load_file("/home/user2/webservicefile/ws2.jl.txt")
    ...
    model.load_weights("tag_best_model.weights")
